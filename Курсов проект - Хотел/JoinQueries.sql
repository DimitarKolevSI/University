...
SELECT floor, AVG(RS.priceForNight) AS avgPrice FROM ROOMS R JOIN RESERVATIONS RS ON RS.roomNumber = R.roomNumber WHERE roomType LIKE 'Room' GROUP BY floor 

SELECT firstName, lastName, COUNT(C.clientsNumber) as NumberOfReservations FROM CLIENTS C JOIN RESERVATIONS R ON C.clientsNumber = r.clientsNumber
